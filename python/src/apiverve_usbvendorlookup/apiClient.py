...
class UsbvendorlookupAPIClientError(Exception):
...
class ValidationError(UsbvendorlookupAPIClientError):
...
class UsbvendorlookupAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"vendorid": {"type": "string", "required": True, "minLength": 4, "maxLength": 4}, "productid": {"type": "string", "required": False, "minLength": 4, "maxLength": 4}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the UsbvendorlookupAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises UsbvendorlookupAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/usbvendorlookup'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise UsbvendorlookupAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[UsbvendorlookupAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns USBVendorLookupData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises UsbvendorlookupAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise UsbvendorlookupAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise UsbvendorlookupAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise UsbvendorlookupAPIClientError(f"Request failed: {str(e)}")
